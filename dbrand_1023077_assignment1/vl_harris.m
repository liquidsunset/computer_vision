...
% Gradienten erzeugen, also die partiellen Ableitungen der ersten Ordnung
% für die X und Y- Richtung in einem Vektor zusammengefasst.
[Ix,Iy] = gradient(I) ;

% Faltet das Quadrat jeweils von Ix^2, Iy^2, Ix*Iy und glättet
% den Output mit einem Gauss-Kernel mit der Standardabweichung von Sigma
% Wird benötigt weil der Response verrauscht ist
...
if nargin > 2
  ...
  % Berechnung der Corner-Response nach Harris
  % Bilden der Determinante (H11.*H22 - H12.^2)
  % und abziehen des Trace H11+H22. Der Trace^2 wird hier
  % noch mit einer Konstante multipliziert zum Verändern des Kontrastes.
  H = (H11.*H22 - H12.^2) - alpha*(H11+H22).^2 ;
else
  % Noble's variation
  % Noble Variante sagt aus, dass wenn Trace/Det klein ist handelt es sich 
  % um eine Ecke. Hier ist es so implementiert, dass wemm Det/Trace groß ist,
  % es sich um eine Ecke handelt. Einfacher, da wird auch beim normalen Harris 
  % Maxima suchen
...
if nargout > 1
  ...
  % Gibt die Spurabbildung (Trace) der Matrix M für die Eigendekomposition einer
  % Matrix an
  tr = H11 + H22 ;

  % Ist die Determinanten der Matrix M für die Eigendekomposition
  dt = H11.*H22 - H12.^2 ;
